...
VarFlow::VarFlow(int width_in, int height_in, int max_level_in, int start_level_in, int n1_in, int n2_in,
	...
	int width = (int)floor(width_in/powf(2.0,(float)(start_level)));
    int height = (int)floor(height_in/powf(2.0,(float)(start_level)));
    ...
    if(width < 1 || height < 1)
    {
        if(width < 1)
        {
              start_level = (int)floor(logf(width_in)/logf(2));
              width = (int)floor(width_in/powf(2.0,(float)(start_level)));
              height = (int)floor(height_in/powf(2.0,(float)(start_level)));
        }
        
        if(height < 1)
        {
              start_level = (int)floor(logf(height_in)/logf(2));
              width = (int)floor(width_in/powf(2.0,(float)(start_level)));
              height = (int)floor(height_in/powf(2.0,(float)(start_level)));
        }
    ...
    int width_end = (int)floor(width_in/powf(2.0,(float)(max_level)));
    int height_end = (int)floor(height_in/powf(2.0,(float)(max_level)));
    ...
    if(width_end < 1 || height_end < 1)
    {
        if(width_end < 1)
        {
              max_level = (int)floor(logf(width_in)/logf(2));
              height_end = (int)floor(height_in/powf(2.0,(float)(max_level)));
        }
        
        if(height_end < 1)
        {
              max_level = (int)floor(logf(height_in)/logf(2));
        }
    ...
    for(i = 0; i < (max_level-start_level+1); i++){
        ...
        imgAfxfx_array[i] = cvCreateImage(cvSize((int)floor(width/powf(2.0,(float)(i))),(int)floor(height/powf(2.0,(float)(i)))), IPL_DEPTH_32F, 1);
        imgAfxfy_array[i] = cvCreateImage(cvSize((int)floor(width/powf(2.0,(float)(i))),(int)floor(height/powf(2.0,(float)(i)))), IPL_DEPTH_32F, 1);
        imgAfxft_array[i] = cvCreateImage(cvSize((int)floor(width/powf(2.0,(float)(i))),(int)floor(height/powf(2.0,(float)(i)))), IPL_DEPTH_32F, 1);
        imgAfyfy_array[i] = cvCreateImage(cvSize((int)floor(width/powf(2.0,(float)(i))),(int)floor(height/powf(2.0,(float)(i)))), IPL_DEPTH_32F, 1);
        imgAfyft_array[i] = cvCreateImage(cvSize((int)floor(width/powf(2.0,(float)(i))),(int)floor(height/powf(2.0,(float)(i)))), IPL_DEPTH_32F, 1);
    
        imgU_array[i] = cvCreateImage(cvSize((int)floor(width/powf(2.0,(float)(i))),(int)floor(height/powf(2.0,(float)(i)))), IPL_DEPTH_32F, 1);
        imgV_array[i] = cvCreateImage(cvSize((int)floor(width/powf(2.0,(float)(i))),(int)floor(height/powf(2.0,(float)(i)))), IPL_DEPTH_32F, 1);
        imgU_res_err_array[i] = cvCreateImage(cvSize((int)floor(width/powf(2.0,(float)(i))),(int)floor(height/powf(2.0,(float)(i)))), IPL_DEPTH_32F, 1);
        imgV_res_err_array[i] = cvCreateImage(cvSize((int)floor(width/powf(2.0,(float)(i))),(int)floor(height/powf(2.0,(float)(i)))), IPL_DEPTH_32F, 1);
...
void _cvConvertScale( const void* srcarr, void* dstarr,
                double scale, double shift )
{
 ...
 #define _cvConvert( src, dst )  _cvConvertScale( (src), (dst), 1, 0 )
int VarFlow::CalcFlow(IplImage* imgA, IplImage* imgB, IplImage* imgU, IplImage* imgV, bool saved_data = false){
    ...
    //Calculate imgAfloat as well as imgBfloat
        ...
        cvResize(imgB, imgBsmall, CV_INTER_LINEAR);
         ...
         _cvConvert(imgAsmall, imgAfloat);
         _cvConvert(imgBsmall, imgBfloat);
    ...
    while(1){
        ...
        gauss_seidel_recursive(k, (max_level - start_level), k, powf(2.0,(float)(k)), imgAfxft_array, imgAfyft_array);
    ...
    if(start_level > 0){
		...
		cvScale(imgU, imgU, powf(2.0, start_level));
		cvScale(imgV, imgV, powf(2.0, start_level));
